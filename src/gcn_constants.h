...
enum ShaderConstantDwordSize : u32 {
...
enum GcnLimits : u32 {
    ...
    kMaxConstantBufferCount = 20, ///< PSSL compiler limit is 20, Default value is 20 Note: Because
                                  ///< API-slots 15-19 are all reserved this value should remain 20
    kMaxStreamOutBufferCount = 4, ///< PSSL compiler limit is 4, Default value is 4
    kMaxUserDataCount =
        16, ///< PSSL compiler limit is 16, count not tracked by the InputResourceOffsets table
    kMaxSrtUserDataCount =
        16, ///< PSSL compiler limit is 16, count not tracked by the InputResourceOffsets table
    kMaxResourceBufferCount =
        4, ///< Maximum number for supported splits for the resource buffer per LCUE instance
