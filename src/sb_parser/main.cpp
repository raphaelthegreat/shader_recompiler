...
static void parseshadercode(const void* code, uint32_t codesize) {
    ...
    Gcn::GcnCodeSlice slice((const u32*)code, (const u32*)code + codesize / 4);
    Gcn::GcnDecodeContext ctx;
	...
	while (1) {
		...
		if (gerr != GCN_ERR_OK) {
			printf(
			    "Failed to decode instruction with: %s\n",
			    gcnStrError(gerr)
			);
			return;
		}
...
static inline void printinputslot(const GnmInputUsageSlot* slot) {
	...
	snprintf(
	    isareg, sizeof(isareg), "s[%u:%u]", slot->startregister,
...
static inline void printgnmheadervs(const GnmVsShader* vsdata) {
	...
	printf(
	    "Constant buffer size DQWORDs: %u\n",
	    vsdata->common.embeddedconstantbufferdqwords
	);
	printf(
	    "Scratch size DWORDs: %u\n",
	...
	printf(
	    "SPI Shader Program Low: %u (0x%08x)\n",
	    vsdata->registers.spishaderpgmlovs,
	    vsdata->registers.spishaderpgmlovs
	);
	printf(
	    "SPI Shader Program High: %u (0x%08x)\n",
	    vsdata->registers.spishaderpgmhivs,
	    vsdata->registers.spishaderpgmhivs
	);
	printf(
	    "SPI Shader Resource 1: %u (0x%08x)\n",
	    vsdata->registers.spishaderpgmrsrc1vs,
	    vsdata->registers.spishaderpgmrsrc1vs
	);
	printf(
	    "SPI Shader Resource 2: %u (0x%08x)\n",
	    vsdata->registers.spishaderpgmrsrc2vs,
	    vsdata->registers.spishaderpgmrsrc2vs
	);
	printf(
	    "SPI VS Out Config: %u (0x%08x)\n",
	    vsdata->registers.spivsoutconfig, vsdata->registers.spivsoutconfig
	);
	printf(
	    "SPI Shader position format: %u (0x%08x)\n",
	    vsdata->registers.spishaderposformat,
	    vsdata->registers.spishaderposformat
	);
	printf(
	    "PA Cl VS Out Control: %u (0x%08x)\n",
	    vsdata->registers.paclvsoutcntl, vsdata->registers.paclvsoutcntl
	);

	const GnmInputUsageSlot* inputslots =
	    gnmVsShaderInputUsageSlotTable(vsdata);
	...
	const GnmVertexInputSemantic* inputstable =
	    gnmVsShaderInputSemanticTable(vsdata);
	...
	const GnmVertexExportSemantic* exportstable =
	    gnmVsShaderExportSemanticTable(vsdata);
...
static inline void printgnmheaderps(const GnmPsShader* psdata) {
	...
	printf(
	    "SPI Shader Z format: %u (0x%08x)\n",
	    psdata->registers.spishaderzformat,
	    psdata->registers.spishaderzformat
	);
	printf(
	    "SPI Shader color format: %u (0x%08x)\n",
	    psdata->registers.spishadercolformat,
	    psdata->registers.spishadercolformat
	);
	printf(
	    "SPI Shader input enable: %u (0x%08x)\n",
	    psdata->registers.spipsinputena, psdata->registers.spipsinputena
	);
	printf(
	    "SPI Shader input address: %u (0x%08x)\n",
	    psdata->registers.spipsinputaddr, psdata->registers.spipsinputaddr
	);
	printf(
	    "SPI Shader input control: %u (0x%08x)\n",
	    psdata->registers.spipsincontrol, psdata->registers.spipsincontrol
	);
	printf(
	    "SPI barycentric control: %u (0x%08x)\n",
	    psdata->registers.spibaryccntl, psdata->registers.spibaryccntl
	);
	printf(
	    "DB shader control: %u (0x%08x)\n",
	    psdata->registers.dbshadercontrol, psdata->registers.dbshadercontrol
	);
	printf(
	    "CB shader mask: %u (0x%08x)\n", psdata->registers.cbshadermask,
	    psdata->registers.cbshadermask
	);

	const GnmInputUsageSlot* inputslots =
	    gnmPsShaderInputUsageSlotTable(psdata);
	...
	const GnmPixelInputSemantic* inputstable =
	    gnmPsShaderInputSemanticTable(psdata);
	for (uint8_t i = 0; i < psdata->numinputsemantics; i += 1) {
		...
		if (input->interpf16 != 0) {
			...
			printf(
			    "Default value f16_hi: %s (%u)\n",
			    gnmStrPixelDefaultValue(input->defaultvaluehi),
			    input->defaultvaluehi
			);
		} else {
			printf(
			    "Is custom: %s\n",
...
static void printgnmshaderfile(const PsslBinaryHeader* hdr) {
	...
	printf(
	    "Embedded constant buffer (DQWORDS): %u\n",
	    common->embeddedconstantbufferdqwords
	);
	printf(
	    "Scratch size per thread (DWORDS): %u\n",
	...
	const uint32_t shadercodeoff =
	    (const uint8_t*)shadercode - (const uint8_t*)hdr;

	printf("\nCode length %u at offset %u", shadercodesize, shadercodeoff);
	parseshadercode(shadercode, shadercodesize);
}
...
static void printshaderparaminfo(const PsslBinaryHeader* hdr) {
	...
	printf(
	    "Num of variables (elements) resources: %u\n",
	    paraminfo->numvariables
	);
	printf(
	    "Num of sampler resources: %u\n", paraminfo->numsamplerresources
	);
	...
	for (uint32_t i = 0; i < paraminfo->numbufferresources; i += 1) {
		...
		printf("Buffer type: %s\n", psslStrBufferType(buf->buffertype));
		printf(
		    "Internal type: %s\n",
	...
	for (uint32_t i = 0; i < paraminfo->numconstants; i += 1) {
		const PsslBinaryConstant* cnst =
		    psslParamConstant(paraminfo, i);
		...
		printf(
		    "Default values (f32): %f %f %f %f\n", cnst->defaultf32[0],
		    cnst->defaultf32[1], cnst->defaultf32[2],
		    cnst->defaultf32[3]
		);
		printf(
		    "Default values (u32): %u %u %u %u\n", cnst->defaultu32[0],
		    cnst->defaultu32[1], cnst->defaultu32[2],
		    cnst->defaultu32[3]
		);
	}
	...
	for (uint32_t i = 0; i < paraminfo->numvariables; i += 1) {
		...
		printf("Array size: %u\n", elem->arraysize);
		printf(
		    "Default value offset OR resource index: %i\n",
	...
	for (uint32_t i = 0; i < paraminfo->numsamplerresources; i += 1) {
		const PsslBinarySamplerState* sampler =
		    psslParamSampler(paraminfo, i);
		printf(
		    "\nSampler %u of %u\n", i + 1,
		    paraminfo->numsamplerresources
		);
	...
	for (uint32_t i = 0; i < paraminfo->numinputattributes; i += 1) {
		const PsslBinaryAttribute* attr =
	...
	for (uint32_t i = 0; i < paraminfo->numoutputattributes; i += 1) {
		const PsslBinaryAttribute* attr =
		    psslParamOutputAttr(paraminfo, i);
		printf(
		    "\nAttribute %u of %u\n", i + 1,
		    paraminfo->numoutputattributes
		);
...
static void printshaderinfo(const void* data) {
	...
	puts("--------------------------------------");
	printf(
	    "Version: %u.%u (revision %u)\n", hdr->vermajor, hdr->verminor,
	    hdr->compiler_revision
	);
	printf(
	    "Association hash: 0x%x 0x%x\n", hdr->association_hash0,
	    hdr->association_hash1
	);
...
std::vector<u8> read_file(std::string_view name) {
...
int main(int argc, char* argv[]) {
    ...
    const auto data = read_file(inputfile);
    printf("Read %zu bytes from %s\n", data.size(), inputfile);

    PsslError perr = psslValidateShaderBinary(data.data(), data.size());
	if (perr != PSSL_ERR_OK) {
        printf(
            "Failed to validate shader binary. %s\n", psslStrError(perr)
		);
        return 0;
	}
